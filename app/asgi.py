...
def _intraday_html(snapshot: dict, symbol: str) -> HTMLResponse:
    ...
    bars = snapshot.get("one_minute_bars") or []

    def v(key):
        item = metrics.get(key)
        return "—" if item is None else escape(str(item))

    rows = "".join(
        "<tr>" + "".join(f"<td>{escape(str(bar.get(key, '')))}</td>" for key in ("time", "open", "high", "low", "close", "volume")) + "</tr>"
        for bar in bars[-60:]
    )
    ...
    cards = "".join(
        f'<div class="card">{label}<br><strong>{v(key)}</strong></div>'
        for label, key in [
            ("当前价", "current"), ("VWAP", "vwap"), ("日内高点", "session_high"),
            ("日内低点", "session_low"), ("15分钟变化%", "change_15m_pct"), ("30分钟变化%", "change_30m_pct")
        ]
    )
    html = f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="index,follow"><title>{title} 分时行情</title><style>body{{font-family:system-ui,sans-serif;margin:24px;line-height:1.5}}main{{max-width:980px;margin:auto}}.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px}}.card{{border:1px solid #ddd;border-radius:10px;padding:12px}}table{{width:100%;border-collapse:collapse}}th,td{{padding:7px;border-bottom:1px solid #eee;text-align:right}}</style></head><body><main><h1>{title} {normalized}</h1><p>更新时间：{as_of} ｜ 数据源：{source}</p><div class="grid">{cards}</div><h2>最近1分钟数据</h2><table><tbody>{rows}</tbody></table></main></body></html>'''
...
def _analysis_html(payload: dict, symbol: str) -> HTMLResponse:
    ...
    minute_rows = "".join(
        "<tr>" + "".join(f"<td>{escape(str(bar.get(key, '')))}</td>" for key in ("time", "open", "high", "low", "close", "volume")) + "</tr>"
        for bar in (intraday.get("one_minute_bars") or [])[-30:]
    )
    daily_rows = "".join(
        "<tr>" + "".join(f"<td>{escape(str(bar.get(key, '')))}</td>" for key in ("trade_date", "open", "high", "low", "close", "volume")) + "</tr>"
        for bar in (daily.get("recent_daily_bars") or [])[-20:]
    )
...
def _analysis_hub_html() -> HTMLResponse:
    html = '''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="index,follow"><title>严进策略实时分析入口</title><style>body{font-family:system-ui,sans-serif;margin:24px;line-height:1.6}main{max-width:760px;margin:auto}.quick{display:block;border:1px solid #ddd;border-radius:12px;padding:16px;margin:12px 0;font-size:20px;text-decoration:none;color:#111}form{display:flex;gap:8px;margin:22px 0}input{flex:1;font-size:20px;padding:12px;border:1px solid #bbb;border-radius:10px}button{font-size:18px;padding:12px 18px;border-radius:10px;border:1px solid #999;background:#fff}small{color:#666}</style></head><body><main><h1>严进策略实时分析入口</h1><p>支持任意 6 位 A 股代码。输入代码后由服务器跳转到对应实时分析页；严进策略参数不在此页修改。</p><form action="/public/analysis/open" method="get"><input name="symbol" inputmode="numeric" pattern="[0-9]{6}" minlength="6" maxlength="6" placeholder="例如 300750" required><button type="submit">打开实时分析</button></form><small>常用快捷入口</small><a class="quick" href="/public/analysis/001309">德明利 001309</a><a class="quick" href="/public/analysis/600110">诺德股份 600110</a></main></body></html>'''
...
def public_analysis_data(symbol: str = Path(...)):
    symbol = _validate_fixed_symbol(symbol)
    return _load_analysis_data(symbol)


@app.get("/public/analysis", response_class=HTMLResponse, include_in_schema=False)
def public_analysis_hub():
    return _analysis_hub_html()


@app.get("/public/analysis/open", include_in_schema=False)
def public_analysis_open(symbol: str = Query(..., min_length=6, max_length=6)):
    symbol = _validate_fixed_symbol(symbol)
    return RedirectResponse(url=f"/public/analysis/{symbol}", status_code=302)


@app.get("/public/analysis/{symbol}", response_class=HTMLResponse, include_in_schema=False)
def public_analysis_view(symbol: str = Path(...)):
    symbol = _validate_fixed_symbol(symbol)
    return _analysis_html(_load_analysis_data(symbol), symbol)
